...
NGRc/MBTiles принадлежат общему app-private каталогу, проект содержит только `shared_underlay_id`. On-demand преобразование тайлов не входит в identity ассета и не создаёт второй payload. Удаление проекта защищает legacy подложки и снимает ссылки; composition sync не управляет ассетами. Closed workspace unlink не создаёт новый `MapBase`. См. [полный контракт](shared-underlays.md).

...
- вынос выбранной точки, линии или границы полигона и отдельные измерения
  ...
  расчётное склонение можно поправить сохраняемой коррекцией прямо в виджете;
...
её наличие. На старой доверенной сборке кнопка переноса должна открывать
предложение обновления. Отдельно проверяется пригодность опубликованного APK:
...
Package Installer явно маршрутизируется из `onActivityResult` обоих экранов.

...
| Background sound | With `background_recording_sound=true`, a health-only zero-distance subscription distinguishes stationary coordinates from missing delivery. While the UI is hidden/screen off and usable fixes remain fresh, a partial wake lock keeps a short notification-stream heartbeat on a fixed 10-second cadence independent of point inserts; an observed persistence failure uses a distinct tone at most once per minute. Missing fresh fixes, a killed process or revoked permission silence the heartbeat and remain the user-visible warning |
...
| Background sound | The same enabled-by-default fixed 10-second heartbeat is driven by fresh usable health callbacks rather than WKT changes, so it continues while stationary and stops when coordinates become stale; commit failure still uses the distinct throttled failure tone |

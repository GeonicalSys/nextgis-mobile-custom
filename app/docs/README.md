...
- вращение карты двумя пальцами только после явного разрешения кнопкой рядом с
  ...
  все включённые raster-подложки доступны с zoom `7`; локальные MBTiles по
  умолчанию делают точный белый прозрачным и растягивают только последний
  масштаб пирамиды через обработку запрошенных тайлов (Настройки → Карта);

...
- ручная синхронизация запускается только для NGW-слоёв активного проекта;
  повторный запуск остаётся защищён от дублирования. При попытке переключить
  проект, загрузить слой или подложку во время sync приложение предупреждает о
  работе синхронизации и предлагает корректно прервать её; после освобождения
  lease выбранное действие продолжается. Запросы во время fill/rebuild по-прежнему
  ждут завершения фоновой операции;

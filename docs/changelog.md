...
- Перенос подложек из старого Debug показывает объём/тайлы и поддерживает
  явную отмену. Зависший cross-package pipe закрывается по inactivity timeout,
  один раз повторяется и всегда освобождает project-operation lease, поэтому
  последующая синхронизация не остаётся заблокированной.

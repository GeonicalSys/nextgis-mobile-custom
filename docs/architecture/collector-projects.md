...
модальное сообщение с просьбой дождаться завершения фоновой операции. Проверка
активной sync повторяется непосредственно перед подготовкой workspace, поэтому
синхронизация, начавшаяся пока пользователь выбирал ресурс, получает то же
предупреждение и безопасное прерывание. Успешный lease подготовки атомарно
переходит из `PROJECT_SWITCH` в `LAYER_FILL` и передаётся foreground service:
между записью registry/metadata и началом fill нет окна для нового sync.
...
- Общий process-wide coordinator удерживает active project identity на всё время
  ручной/периодической sync, layer fill и schema rebuild. Во время sync попытка
  switch/create/rename/delete или загрузки слоя/подложки показывает предупреждение
  и может корректно прервать sync: read-only HTTP закрывается немедленно,
  tracked SQLite apply откатывается, а уже отправленный server write получает
  однозначный ответ. Действие продолжается только после освобождения lease.
  Во время fill/rebuild сохраняется модальное ожидание; повторная полная
  sync того же проекта также не запускается. Зависимая цепочка
...
Во время stream экран показывает source index, объём и число тайлов. Явная
отмена или отсутствие новых байтов закрывает descriptor и только после выхода
I/O освобождает `UNDERLAY_MIGRATION`; stalled source повторяется один раз.
Таким образом незавершённый перенос не может бессрочно блокировать data sync.

...
- Подготовленный выпуск: Lisa/Belka Release `3.1.2.21` / `versionCode 215`,
  Lisa Debug использует `3.1.2.21` / `versionCode 216`; maplib сообщает соответствующее
...
На 2026-09-14 сервер отдаёт `3.1.2.17/212`; эта версия уже содержит exporter
и может обновить старый `3.0.3.5/183`. Наличие более нового локального APK
`3.1.2.21/216` не является условием переноса; серверный APK всё равно проходит

...
- Во время синхронизации загрузка слоя/подложки и смена проекта теперь показывают
  предупреждение с явным прерыванием sync; действие продолжается после остановки
  worker и освобождения lease. Spinner учитывает весь последовательный account-pass
  и не гаснет из-за пропущенного broadcast.

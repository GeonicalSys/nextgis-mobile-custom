...
- новый скетч начинается сразу после выбора слоя: точка или первый узел линии/
  ...
  кнопки по одной отменяют и возвращают добавление или перенос точки линейки;
...
- запись трека и геометрии обходом при валидном движении до 160 км/ч без выбора
  ...
  preference звукового контроля при скрытом UI подтверждает только успешно
  сохранённые точки коротким сигналом не чаще раза в 10 секунд, а явную ошибку
  сохранения — отдельным сигналом не чаще раза в минуту; отзыв разрешения

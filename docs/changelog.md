...
- Камера MapLibre не опускается ниже `7.5`, а все включённые raster-подложки
  доступны с zoom `7`. Многогигабайтная sidecar-генерация удалена: прозрачный
  белый, дырки пирамиды и ограниченный underzoom локальных MBTiles выполняются
  только для запрошенных экраном тайлов через loopback server. Исходный MBTiles
  не переписывается; устаревшие производные копии очищаются.

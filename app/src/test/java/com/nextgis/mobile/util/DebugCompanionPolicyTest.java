...
public class DebugCompanionPolicyTest {
    private JSONObject manifest() throws Exception {
        return new JSONObject().put("schemaVersion", 1).put("applicationId", DebugCompanionPolicy.PACKAGE)
                .put("flavor", "debug").put("channel", "debug").put("versionCode", 217).put("versionName", "3.1.2.22")
                ...
                .put("apkUrl", "https://apps-geonical.ru/lisa-mobile/debug/releases/217/debug.apk");
    }
    @Test public void acceptsOnlyDebugPackageChannelFlavorAndPinnedCertificate() throws Exception {
        assertEquals(217, new DebugCompanionPolicy.Manifest(manifest(), 36).code);
    ...
    @Test public void rejectsUnsafeOrWrongReleaseUrls() throws Exception {
        for (String url : new String[]{"http://apps-geonical.ru/lisa-mobile/debug/releases/217/debug.apk",
                "https://apps-geonical.ru/lisa-mobile/lisa/releases/217/debug.apk",
                "https://apps-geonical.ru/lisa-mobile/debug/releases/216/debug.apk",
                "https://apps-geonical.ru/lisa-mobile/debug/releases/217/../217/debug.apk",
                "https://apps-geonical.ru/lisa-mobile/debug/releases/217/%2e%2e/a.apk",
                "https://apps-geonical.ru/lisa-mobile/debug/releases/217/debug.apk?next=other",
                "https://other.example/lisa-mobile/debug/releases/217/debug.apk"}) {
    ...
    @Test public void checksInstalledAndArchiveSignersAndRequiresUpgrade() throws Exception {
        ...
        DebugCompanionPolicy.validateApk(m, DebugCompanionPolicy.PACKAGE, "debug", 217, "3.1.2.22", 216, trusted, trusted);
        for (int variant = 0; variant < 4; variant++) {
            try {
                DebugCompanionPolicy.validateApk(m, DebugCompanionPolicy.PACKAGE, "debug", 217,
                        variant == 0 ? "different" : "3.1.2.22", variant == 1 ? 217 : 216,

...
- Подготовлен выпуск Lisa/Belka `3.1.2.22` / `versionCode 216` и Lisa Debug
  `3.1.2.22` / `versionCode 217`. В выпуск входят зелёный значок видимости,
  быстрое безопасное прерывание sync перед сменой проекта и загрузкой слоя,
  кольцо прогресса вокруг иконки sync, внутренние якоря подписей полигонов и
  сплошной белый светлый фон карты. Root закрепляет Merge Commit maplib PR #32
  `ffff44f` и maplibui PR #20 `56fd1585`.

- Вокруг крутящейся иконки sync показывается кольцо без процентов: вес слоя
  учитывает локальные правки, внутри слоя — push/TUS, байты snapshot и apply
  объектов; неизвестный остаток не имитируется. Composition и LayerFill в
  кольцо не входят.

- Во время синхронизации загрузка слоя/подложки и смена проекта теперь показывают
  предупреждение с явным прерыванием sync; create/rename/delete и финальная
  подготовка Collector workspace используют тот же gate. Read-only HTTP
  закрывается сразу, tracked apply откатывается целиком, а уже отправленный
  server write безопасно завершает только текущий запрос. Ручной sync использует
  один serial account-pass без дублирующих framework-запусков, а spinner
  двусторонне восстанавливается по worker/lease state после пропущенного broadcast.

- Для списка слоёв карты документированы зелёный значок включённой видимости,
  неизменный значок выключенного слоя и проверка обеих тем после прокрутки списка.

- Якоря подписей Polygon и MultiPolygon перенесены с среднего арифметического
  вершин на внутреннюю точку заливки с учётом отверстий; для MultiPolygon
  выбирается наибольшая пригодная часть.

- Исправлена реализация светлого фона карты: MapLibre теперь рисует
  `background-color: #FFFFFF` вместо повторения белой текстуры 1×1 пиксель.
  Smoke проверяет равномерность на нескольких масштабах и после смены режима.
  Root закрепляет Merge Commit maplib PR #31 `63063ae`.

## 2026-09-18

- Камера MapLibre не опускается ниже `7.5`, все включённые raster-подложки
  доступны с zoom `7`, а режим фона карты «Светлый» теперь строго белый
  (`#FFFFFF`). Экспериментальные proxy и преобразования raster-тайлов удалены;
  локальные MBTiles снова подключаются напрямую.
...
- Слои `SYNC_NONE` при синхронизации сравнивают отфильтрованное число объектов
  с сервером и при расхождении (если серверный count > 0) пересобирают слой
  полным snapshot; нулевой или недоступный count локальные данные не стирает.
- Курсор GPS больше не пишет каждую точку, autopan и map lock в HyperLog;
  эти строки остаются только в Logcat при `DEBUG_MODE`. `GPS health` и журнал
  трека не меняются. Чекбокс «Расширенное логирование» (`verbose_log`) возвращает
  полный GNSS/NMEA дамп в файл, включая координаты.
- Внешний GNSS при выбранном приёмнике остаётся подключённым в фоне и во сне:
  сессия не закрывается из-за `onPause` карты, процесс держит
  `ExternalGnssService` (`location|connectedDevice`) и общий `PARTIAL_WAKE_LOCK`.
- Вынос не считает слегка будущий GNSS timestamp потерей фикса (допуск 1 с).
  Для системного GPS порог «Ожидание GPS» 3 с; для native NMEA — 8 с, как у курсора.
- Native NMEA не отбрасывается как чип телефона при живом `lastMock`; при
  `gnss_input=external` карта не подписывается на Network. HUD внешнего GNSS
  справа от компаса, короткие подписи «фикс» / «плав» / «авто».
- Карта не смешивает свежий GPS (чип, mock или native NMEA) с Wi‑Fi/сотой: сеть только
  если GNSS нет или старше 8 с. Mock внешнего приёмника вытесняет чип;
  заглушка GPS Connector 47 м без extras не сменяет hdop-фикс. Трек и обход
  пишут координаты приёмника без пешеходного smoother, не грубее 2 с / 1 м.
- Внешний GNSS читается в приложении из NMEA (Bluetooth Classic/LE, USB, TCP/IP)
  без GPS Connector и Mock Location. HUD справа от компаса показывает фикс/плав/авто,
  спутники и HDOP; точность круга берётся из GST, иначе из HDOP.
- «Открыть локальный» кладёт NGRc и raster MBTiles (включая ZIP с NGRc-конфигом
  или одним MBTiles) в общее хранилище подложек, как «Новая подложка из файла».
- Меню добавления слоя начинается с «Загрузить проект»: поиск группы Веб ГИС с
  ключом `lisa` и список Collector-проектов внутри неё. «Добавить слой NGW по
  URL» скрыт, обработчик сохранён.
...
- У курсора GPS поверх круга точности рисуется геодезический сектор направления
  телефона: ось — истинный курс (`магнитный heading + D_WMM`, без коррекции
  выноса), ширина следует за неопределённостью heading (датчик, джиттер курса,
  магнитометр; раскрытие быстрее сужения), длина 8 м. Нет компаса или
  устаревший heading скрывают только сектор. Иконка stand/go не меняется.
  App PR закрепляет maplib `2994901` (Draft PR #24) до Merge Commit библиотеки.

- Кнопки масштаба карты перенесены к правому краю. Пункты меню азимута
  сокращены до «От себя до точки» и «Между двумя точками». «Сброс» коррекции
  склонения всегда справа от «+», без сжатия ряда −/значение/+.

- Кнопка измерения азимута использует белый PNG исходного символа: чёрные
  пиксели перекрашены в белый с сохранением альфы, векторная звезда удалена.
  К расчётному WMM2025-склонению добавлена сохраняемая коррекция
  `stakeout_declination_correction`; в виджете строка склонения показывает
  эффективное `D + C`, коррекция остаётся отдельной строкой. Подсказки
  перетаскивания из панели убраны.
- Перенос подложек из старого Debug перенесён в «Хранилище подложек»
  («Импортировать из старого приложения»); пункт на экране «Проект» убран.
  Кнопка видна только в Geonical при установленном доверенном Debug и скрыта
  в режиме выбора подложки для слоя. Companion после установки открывает каталог.

- Восстановлен потоковый импорт стандартного NGRc с `Mapnik.json` и JPEG/PNG/WebP
  в `Mapnik/`; сохранены исходные байты, схема и полный хеш архива. Удаление
  слоя после ошибки/отмены перенесено на main thread перед обращением к MapLibre.
- Исправлена проверка отсутствующего exporter старого Debug: используется
  фактический результат `PackageManager`, включая enabled/exported состояния.
  Повторно проверено ограничение публичного Debug-канала `3.1.2.17/212`.
  Обновлены контракты импорта, rendering, companion, local packs и smoke.
- Устранён повторный permission-диалог companion при смене фокуса: busy
  охватывает UI-переход, а завершение внешнего экрана приходит через activity
  result. Серверный `3.1.2.17/212` уже содержит exporter и подходит для старого
  Debug; новая публикация не является условием этого переноса.
- Root закрепляет удалённый Merge Commit maplib PR #23 `96db542` вместо
  тестового pin `a5603fb7`; деревья идентичны, предыдущие GPS/обход, каталог
  и marker-icons-64 включены. Для app #29 используется Squash Merge.
  По запросу пользователя версии не повышаются, APK не публикуются.

## 2026-09-13

- Исправлены AppCompat-тема экрана хранилища и ссылки при объединении одного
  NGRc в разных форматах. Описаны сохранение проектных масштабов, восстановление
  прежних неверных ссылок и регрессии с прерыванием move/redirect; актуализирована
  цепочка уже объединённых PR и порядок доставки исправлений.
- «Хранилище подложек» вынесено из общих настроек на первый экран настроек,
  рядом с «Проект»; явный Toolbar и системные insets устраняют перекрытие
  первой строки на Android 16. Обновлены маршрутизация, registry и smoke.
